...
float callCharge;
    ...
    (startTime - static_cast<int>(startTime) > .60 ) ? cout << "You entered an invalid time! There are too many minutes, please try again and enter a value between 0 - 59" << endl :
    (startTime > 24.00) ? cout << "You entered more time than there is in a day! Please try again and enter the time correctly." << endl :
    (startTime < 0 ) ? cout << "You entered a negative time! This is invalid, please try again and enter the time correctly" << endl :

    cout << "" << endl;

    if (startTime - static_cast<int>(startTime) > .60 || startTime > 24.00 || startTime < 0) {
        cout << endl << "An invalid time was entered and the program has ended.";
        return 0;
    }
    ...
    (startTime <= 6.59) ? callCharge = totalMinutes * 0.12 :

	(startTime > 6.59 && startTime <= 19.00) ? callCharge = totalMinutes * 0.55 : callCharge = totalMinutes * 0.35;
